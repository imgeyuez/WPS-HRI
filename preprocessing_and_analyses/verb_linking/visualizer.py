...
if input_text.strip() and (llm_annotations or manual_annotations):
    llm_matched = next(
        (entry for entry in llm_annotations["results"] if input_text.strip() in entry["paragraph"]),
        None,
    ) if llm_annotations else None

    manual_matched = next(
        (entry for entry in manual_annotations["results"] if input_text.strip() in entry["paragraph"]),
        None,
